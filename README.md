...
- **Unified calendar layer**: on Google Calendar, `Cmd/Ctrl+K` opens a layer palette to toggle or focus any calendar, switch accounts, and add more sources, plus single keys (`c` create, `t` today, `←`/`→` (or `j`/`k`) previous/next, `d`/`w`/`m` day/week/month) and a persistent nudge to keep adding accounts and Outlook/iCal feeds into one view. Two simple add flows: to add another Google account, Inbox Keys opens Calendar's own "Subscribe to calendar" pane in a new tab and keeps this tab open with a short guide and a Refresh button (type the account's email in Google's box, approve the permission email it sends, then Refresh); to add an Outlook/iCal calendar, paste its published `.ics` link. The merge is Google's own (calendar subscription and sharing); Inbox Keys just drives the UI and reads no event data.
