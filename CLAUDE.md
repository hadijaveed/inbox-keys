...
Open link or attachment (Cmd+O, threadView) is a ranked decision, not first-match. `extractOpenCandidates(scope)` in `gmail.js` collects attachment chips (`SEL.attachmentChip`: `span.aZo, [download_url]`, filename from `SEL.attachmentName` `.aV3` or the `download_url` "mime:filename:url" attribute) and body links (visible `a[href]` inside `SEL.messageBody`, excluding `mailto:`/`tel:`/`javascript:`/`#`), classifying links inside `SEL.signature` (`.gmail_signature`) or `SEL.quotedText` (`.gmail_quote`, `.adL`) as weak. Duplicate destinations dedupe to the best-ranked occurrence; google.com/url redirect wrappers are unwrapped for display/dedupe only — the original anchor is always what gets realClicked. Decision rule: 0 candidates → toast; exactly 1 candidate, or exactly 1 strong (attachment or body link) above any signature/quoted noise → open it directly; anything else → `palette.choose()` picker (the Cmd+K overlay in one-shot mode: same fuzzy filter, arrows/Enter/Escape via the engine's `palette.isOpen()` routing, groups Attachments → Links → Signature/Quoted). So one attachment beats six signature links with zero extra keystrokes, and two attachments ask which. Caveat: the attachment-chip selectors come from Gmail's long-stable attachment DOM but were NOT verified live on this box (no smoke profile); if Cmd+O misses attachments in the field, probe `span.aZo` / `[download_url]` / `.aV3` first. Attachments open via realClick on the chip's ANCHOR (realClick has no hit-testing, and Gmail's open handler lives on the anchor filling the chip, not the aZo wrapper) — Gmail's previewer, never URL navigation. If no chips match, labeled per-attachment controls ("Download attachment …") are a structural fallback, excluding toolbars so "Download all" can't masquerade as a file. While the picker (or palette) is open the engine also consumes Mod+O/Mod+U so they can't fall through to Chrome's file dialog / view-source.

Forward (f, threadView) is message-aware because of a Gmail scoping trap: the bottom inline "Forward" `.ams` link forwards ONLY the LAST message of the conversation, with only THAT message's attachments — the quoted text of the whole thread rides along, so the draft looks complete while attachments on earlier messages are silently dropped (the reported "recipients didn't get the files"). `forwardThread(scope)` in `gmail.js` now takes the threadnav-focused card (wired in `commands.js` like reply-all). Decision rule: cursor on an EARLIER card → `forwardMessage(card)` directly; default (last card) → if no other card has visible attachment chips AND every card is expanded, click the inline Forward as before (single keystroke); otherwise open a `palette.choose()` picker listing every message (attachment-bearing ones ranked first with counts; collapsed cards say "attachments unknown" since their chips aren't in the DOM). `forwardMessage(card)` is the only path with per-message identity: expand the card if collapsed (header click), `hover()` the header (the kebab is hover-gated), open the card-scoped kebab (`SEL.messageMenuButton`, BOTH label spellings "More message options"/"More email options", `closest(SEL.toolbars)` excluded so the conversation-toolbar "More email options" can never be hit), then click the menu item whose label is EXACTLY "Forward" — never a prefix match, because "Forward all" and "Forward as attachment" have different attachment semantics. The old `labeledButton(/^Forward\b/)` document-wide fallback is gone for the same reason; do not reintroduce it (sim test pins its absence). Hardening (each pinned by a test): a collapsed "N older messages" stack (`threadnav.hiddenMessageControls()`) also makes the target ambiguous — the picker gets an extra "expand to inspect attachments" row that reveals the stack and re-runs forward (each expansion strictly grows the card list, so no loop); the draft guard is `threadInlineDraft()`, scoped to `role=main`, so a background standalone compose popup or docked Chat box (both match `SEL.composeBody` document-wide) never blocks f — only a blurred INLINE draft refuses, with a "Close the open draft first" toast; `forwardMessage` snapshots showing menus before clicking the kebab and only accepts a Forward item from a menu that was not already showing (a stale menu could belong to another card), and its give-up path only re-clicks the kebab if a new menu is actually showing (a blind second click would OPEN a menu after polling stopped); a detached card (Gmail re-rendered under the picker) fails fast with "The thread changed — press f again". Every kebab-path failure toasts stage-specifically ("Forward (message menu)", "(message menu item)", "(expand message)"). Caveat: the kebab flow was built from Gmail's known DOM but NOT verified live on this box; if per-message forward fails in the field, probe the two kebab labels first.
